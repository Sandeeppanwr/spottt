...
export async function POST(req) {
  try {
    ...
    // ✅ Dev mode: return mock to avoid 429 errors
   if (process.env.NODE_ENV === "development") {
  const generateMockDescription = (prompt) => {
    return `This event is about ${prompt}. Attendees will learn practical skills and gain insights related to ${prompt}. It is designed to be engaging, informative, and interactive for participants interested in ${prompt}.`;
  };

  return NextResponse.json({
    title: `Exciting Event on ${prompt}`,
    description: generateMockDescription(prompt),
    category: "tech",
